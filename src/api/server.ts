...
export function startWebServer(db: Database.Database, port: number = 3000): void {
    const server = http.createServer(async (req, res) => {
        ...
        try {
            ...
            if (url.pathname === '/api/people' && method === 'PUT') {
                ...
                const before = db.prepare('SELECT * FROM people WHERE id = ?').get(id) as any;
                if (!before) {
                ...
                const updateTx = db.transaction(() => {
                    ...
                    logAuditAction(
                        ...
                        JSON.stringify({ canonical_name, gender, state, research_status, updated_at: now }),
                        justification
                    );
                });
